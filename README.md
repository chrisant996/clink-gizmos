...
[down_suggestion_list.lua](down_suggestion_list.lua) | Augments the <kbd>Down</kbd> key to be able to also show the suggestion list.  Refer to the usage information in the script file for details.
[filter_short_history.lua](filter_short_history.lua) | Adds a setting to skip adding lines to history unless they meet a minimum specified length.  For example, `clink set history.minimum_length 3`.  Refer to the usage information in the script file for details.
[fully_qualify.lua](fully_qualify.lua) | Use <kbd>Alt</kbd>-<kbd>Ctrl</kbd>-<kbd>Q</kbd> to fully qualify the path name under the cursor.  Refer to the usage information in the script file for details.
...
[fzf_rg.lua](fzf_rg.lua) | Adds support for using [ripgrep](https://github.com/BurntSushi/ripgrep) and [fzf](https://github.com/junegunn/fzf) together to search files and do fuzzy filtering on the results.  Available as a key binding, or by running `fzf_rg.cmd` at the command line.  Refer to the usage information in the script file for more information.  (This is a copy from the [clink-fzf](https://github.com/chrisant996/clink-fzf) repo.)
...
[killringbar.lua](killringbar.lua) | Optionally shows a bar above the prompt, containing the current kill-ring text.  Refer to the usage information in the script file for details.
